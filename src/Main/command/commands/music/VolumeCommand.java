...
public class VolumeCommand implements ICommand{
	...
	public void handle(CommandContext ctx) {
		...
		try {
			...
			channel.sendMessage("Invalid command use `!volume [# 0-100]`").queue((message) -> {
		...
		if (args.size() == 1) {
			...
			if(Integer.parseInt(n1)<0) {
				channel.sendMessage("You can't use negative value! `!volume [# 0-100]`").queue((message) -> {
					message.delete().queueAfter(delTime, TimeUnit.SECONDS);
				});
				return;
			}
			if(Integer.parseInt(n1)>100) {
				channel.sendMessage("You can't set volume more than 100 `!volume [# 0-100]`").queue((message) -> {
					message.delete().queueAfter(delTime, TimeUnit.SECONDS);
				});
				return;
			}
